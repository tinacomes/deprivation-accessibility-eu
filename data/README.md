...
| Eurostat **Census 2021 grid** | EU-wide 1 km demographics (GEOSTAT successor): total population, sex, broad age (< 15 / 15–64 / ≥ 65), employed persons (voluntary), country of birth, prior residence — the 13 variables of EU Reg. 2018/1799. Ingested by `ingest/census.py`. The v1.0 archive delivers the attribute table as **`ESTAT_Census_2021_V1-0.gpkg`** (GeoPackage) beside a total-population GeoTIFF, the ESMS PDF, a `read.me` and a nested INSPIRE country delivery; the member, id column and variable codes all live in `sources.census` (`config/defaults.yaml` — the **variable codes** are still `verify: TODO`) | landing page: https://ec.europa.eu/eurostat/web/gisco/geodata/population-distribution/population-grids · files: https://gisco-services.ec.europa.eu/census/2021/ and https://gisco-services.ec.europa.eu/pub/population-grid/2021/ | CC-BY 4.0 (© European Union) | 1 km (**broadcast** onto the 100 m analysis grid — see methods.md §6.1) | EPSG:3035, INSPIRE `GRD_ID` |
