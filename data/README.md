...
| Eurostat **Census 2021 grid** | EU-wide 1 km demographics (GEOSTAT successor): total population, sex, broad age (< 15 / 15–64 / ≥ 65), employed persons (voluntary), country of birth, prior residence — the 13 variables of EU Reg. 2018/1799. Ingested by `ingest/census.py` from release **V3** (`Eurostat_Census-GRID_2021_V3.zip`), which ships one wide table as GeoPackage, **CSV** (the member we read), Parquet and GeoTIFF. Codes per its read.me: `T`, `M`, `F`, `Y_LT15`, `Y_1564`, `Y_GE65`, `EMP`, `NAT`/`EU_OTH`/`OTH`, `SAME`/`CHG_IN`/`CHG_OUT`, `LAND_SURFACE`, `POPULATED`; `-8888` = confidential, `-9999` = unavailable. Member, id column, codes and sentinels all live in `sources.census` (`config/defaults.yaml`) | landing page: https://ec.europa.eu/eurostat/web/gisco/geodata/population-distribution/population-grids · files: https://gisco-services.ec.europa.eu/census/2021/ and https://gisco-services.ec.europa.eu/pub/population-grid/2021/ | CC-BY 4.0 (© European Union) | 1 km (**broadcast** onto the 100 m analysis grid — see methods.md §6.1) | EPSG:3035, INSPIRE `GRD_ID` |
