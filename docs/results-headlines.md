...
population: elasticity **−0.19** (country-clustered SE 0.023, wild
cluster bootstrap p = 0.0002, R² ≈ 0.44), robust to country fixed
effects, leave-one-out city deletion (envelope −0.20 to −0.18), robust
estimators (Huber, median regression) and exclusion of the
emergency-desert group. Mean emergency deprivation shows **no detectable
size gradient** (−0.05, p ≈ 0.37) — and the TOST bounds it: the
emergency elasticity lies within **±0.13** (equivalence at the stricter
±0.10 bound is not shown, so say "no detectable scaling, bounded within
±0.13", never "proven flat"). The difference between the two slopes is
itself significant (paired regime test, +0.14, wild p ≈ 0.004).
...
countries across regions): the emergency Gini (Cohen f 0.77,
p_perm = 0.002), the divergence gap (p_perm = 0.012), the coupling ρ
(p_perm = 0.0016) and both compounding measures (p_perm ≤ 0.002) differ
by macro-region. **CEE compounds**: highest emergency inequality
(median Gini ≈ 0.54) *and* the strongest everyday–emergency coupling
(median ρ ≈ 0.55) — the same populations tend to carry both
deprivations. **The West's inequality is everyday-side** (negative
divergence gap); the North's is emergency-side. The everyday Gini's
apparent regional contrast does **not** survive country-level testing
(p_perm = 0.33 despite city-level p = 0.001) — do not claim it.

### H4 — No city types; regional gradients plus one desert group

k-means and Ward agree on a single 62-vs-5 split: **București, Rīga,
Vilnius, Tallinn, Ljubljana** — the emergency-desert capitals, driven by
their beyond-threshold emergency travel-time tails. The split is real
...
(HH share at the median split) exceeds the 25 % independence benchmark
in most cities; coupling ρ spans ~0 to 0.74. The continuous
`compounding_intensity` (which removes the threshold's leverage) shows
the same regional pattern (p_perm = 0.002). Per-city assumption
...
| flip cells | Cells whose typology class changes under any variant — the spatial footprint of an assumption. |
