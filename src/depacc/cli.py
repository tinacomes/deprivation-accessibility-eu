...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "make-city":
        ...
        if path.exists():
            ...
            return 1
        from depacc.ingest.osm import geofabrik_country_url

        pbf_url = geofabrik_country_url(args.country)
        if pbf_url is None:
            print(f"no Geofabrik country mapping for '{args.country}': add the "
                  f"country to depacc.ingest.osm.GEOFABRIK_COUNTRY, or write "
                  f"the config by hand with an explicit sources.osm_pbf.url")
            return 1
        path.write_text(
            f"# Tier-1 city (generated by `depacc make-city`).\n"
            f"# Facilities via Overpass; travel times via r5 over the country\n"
            f"# Geofabrik extract, clipped to the FUA window (methods.md §5).\n"
            f"# Substitute a tighter regional extract when one exists — see\n"
            f"# hamburg.yaml for the pattern.\n"
            f"city:\n"
            f"  id: \"{city_id}\"\n"
            f"  name: \"{args.name}\"\n"
